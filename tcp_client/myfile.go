...
func (f *FileOps) WriteLine(str string) (n int) {
	var err error
	var n_write int
	...
	n_write, err = f.fileptr.Write([]byte(str))
	if nil != err {
...
func (f *FileOps) FileConfig2Client(path string) *ClientConfig {
	var file_line string
	var ip string
	var port string
	var thread string
	...
	for {
		...
		} else if strings.Contains(file_line, "PORT") {
			if i := strings.Index(file_line, "="); i > 0 {
				line_bytes := []byte(file_line)
				port = string(line_bytes[i+1:])
				conf.server_port = trimString(port)
			}
		} else if strings.Contains(file_line, "THREAD") {
			if i := strings.Index(file_line, "="); i > 0 {
				line_bytes := []byte(file_line)
				thread = string(line_bytes[i+1:])
				conf.client_thread, _ = strconv.Atoi(trimString(thread))
			}
		}
